...
function cleanUpWiFiInfoObj(info) {
    if (info.encrypted && !info.security) {
        info.security = "WEP";
    }
    delete info.encrypted;
}
...
function parseIwlist(str) {
    ...
    for (var i = 0, l = out.length; i < l; i++) {
        ...
        if (line.match(encryption_key)) {
            info.encrypted = (encryption_key.exec(line)[1]).trim().toLowerCase() === "on";
        }
        for (var type in security) {
            if (line.match(security[type])) {
                info.security = type;
            }
        }
    }
